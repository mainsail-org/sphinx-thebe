...
# TODO: mir - satyrn config
def init_thebe_satyrn_config(app, env, docnames):
    thebe_config = app.config.thebe_config
    defaults = {
        "always_load": True,
        "selector": ".thebe",
...
def init_thebe_core(app, pagename, templatename, context, doctree):
    ...
    thebe_config = f"""
        thebe_selector = "{ app.config.thebe_config['selector'] }"
        thebe_selector_input = "{ app.config.thebe_config['selector_input'] }"
        thebe_selector_output = "{ app.config.thebe_config['selector_output'] }"
    """
    app.add_js_file(None, body=thebe_config)
    app.add_js_file(filename="sphinx-thebe.js", **opts)


# TODO: mir, see if necessary
def init_satyrn_core(app, pagename, templatename, context, doctree):
    """Load thebe assets if there's a thebe button on this page."""
    config_thebe = app.config["thebe_config"]
    if not _do_load_thebe(doctree, config_thebe):
        return
...
def update_satyrn_context(app, doctree, docname):
    """Add thebe config nodes to this doctree."""
    config_thebe = app.config["thebe_config"]
    if not _do_load_thebe(doctree, config_thebe):
        return
    ...
    satyrn_url = "http://127.0.0.1:12345"
    satyrn_token = "letmein"

    # Update the doctree with some nodes for the thebe configuration
    thebe_html_config2 = f"""
        <script type="text/x-thebe-config">
        {{
            bootstrap: true,
            codeMirrorConfig: {{
    ...
    doctree.append(nodes.raw(text=thebe_html_config2, format="html"))
    doctree.append(
        nodes.raw(text=f"<script>kernelName = '{kernel_name}'</script>", format="html")
    )
...
def setup(app):
    ...
    # TODO: mir - satyrn config
    # Update the doctree with satyrn-specific information if needed
    app.connect("doctree-resolved", update_satyrn_context)

    # Load the JS/CSS assets for thebe if needed
    # app.connect("html-page-context", init_thebe_core)

    # TODO: mir, satyrn core
    app.connect("html-page-context", init_satyrn_core)
